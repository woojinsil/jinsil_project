...
class ViewController: UIViewController,  UITextFieldDelegate {
    ...
    @IBAction func loginPressed() {
        ...
        let urlString: String = "http://condi.swu.ac.kr/student/M06/login/loginUser_last.php"
        guard let requestURL = URL(string: urlString) else {
            return }
        self.labelStatus.text = " "
        var request = URLRequest(url: requestURL)
        request.httpMethod = "POST"
        let restString: String = "userid=" + loginUserid.text! + "&password=" + loginPassword.text!
        ...
        do {
            ...
            if !(200...299 ~= response.statusCode) {
                print ("HTTP Error!")
                return }
            guard let jsonData = try JSONSerialization.jsonObject(with: receivedData, options:.allowFragments) as? [String: Any]
                else {
